...
[string]   $version               = "v3.16.0915"
[string]   $settingsFile          = "lang-test.ini"
...
[array]$script:qaChecks = ('c-acc-01-local-users','c-acc-02-local-admin-name','c-acc-03-local-admins','c-acc-04-local-groups','c-acc-05-service-logon-accounts',
...
$script:lang        = @{}
$script:appSettings = @{}
$script:appSettings['StaticRoute01'] = ('192.168.1.10', '255.255.255.0', '192.168.1.1')
$script:appSettings['StaticRoute02'] = ('192.168.2.10', '255.255.255.0', '192.168.2.1')
...
$script:qahelp['acc02']='<xml><description>Checks to see if the local default accounts have been renamed. The "Administrator" and "Guest" accounts should be.</description><pass>Local admin account has been renamed</pass><fail>A local admin account was found that needs to be renamed</fail><applies>All</applies></xml>'
...
$script:qahelp['vhv01']='<xml><description>Check that the latest HyperV tools are installed</description><pass>HyperV tools are up to date</pass><fail>HyperV tools can be upgraded</fail><manual>Unable to check the HyperV Tools upgrade status</manual><na>Not a virtual machine</na><applies>Virtuals</applies></xml>'
