...
[array]$script:qaChecks = ('c-acc-01-local-users','c-acc-02-local-account-names','c-acc-03-local-admins','c-acc-04-local-groups','c-acc-05-service-logon-accounts',
                           ...
                           'c-com-08-bladelogic-agent-installed','c-drv-01-system-drive-size','c-drv-02-min-drive-freespace','c-drv-03-pagefile-size-location',
                           'c-drv-04-cd-dvd-drive','c-drv-05-shared-folders','c-drv-06-san-storage','c-drv-07-disk-management-agent','c-drv-08-drive-ntfs-format',
                           'c-hvh-01-server-core','c-hvh-02-no-other-server-roles','c-hvh-03-vm-location','c-hvh-04-integration-services','c-net-01-no-ipv6',
...
$script:lang['Script-Error'] = 'SCRIPT ERROR'
$script:lang['Warning'] = 'Warning'
$script:lang['Name'] = 'All Drives Are NTFS format'
<#
    DESCRIPTION: 
...
$script:lang        = @{}
$script:appSettings = @{}
$script:appSettings['IgnoreThesePorts'] = ('135', '139', '443', '445', '902', '912', '8192')
$script:lang['Error'] = 'Error'
$script:lang['Fail'] = 'Fail'
$script:lang['Manual'] = 'Manual'
$script:lang['Not-Applicable'] = 'N/A'
...
$script:qahelp['acc07']='<xml><description>Checks the builtin group memberships to make sure specific users or groups are members. If there is only one entry in "GroupMembers", then "AllMustExist" will be forced to "TRUE" This is check 1 of 3 that can be used to check different groups.</description><pass>No additional users exist / Additional users exist</pass><warning>Invalid group name</warning><fail>Additional users exist</fail><applies>All</applies></xml>'
$script:qahelp['acc08']='<xml><description>Checks the builtin group memberships to make sure specific users or groups are members. If there is only one entry in "GroupMembers", then "AllMustExist" will be forced to "TRUE" This is check 2 of 3 that can be used to check different groups.</description><pass>No additional users exist / Additional users exist</pass><warning>Invalid group name</warning><fail>Additional users exist</fail><applies>All</applies></xml>'
$script:qahelp['acc09']='<xml><description>Checks the builtin group memberships to make sure specific users or groups are members. If there is only one entry in "GroupMembers", then "AllMustExist" will be forced to "TRUE" This is check 3 of 3 that can be used to check different groups.</description><pass>No additional users exist / Additional users exist</pass><warning>Invalid group name</warning><fail>Additional users exist</fail><applies>All</applies></xml>'
...
$script:qahelp['com08']='<xml><description>Check BladeLogic monitoring agent is installed, and that the correct port is listening. Also checks that the USERS.LOCAL file is configured correctly.</description><pass>BladeLogic agent found, and file confgiured</pass><fail>BladeLogic agent not found, install required / Required port not listening / USERS.LOCAL not configured / USERS.LOCAL not found</fail><applies>All</applies></xml>'
...
$script:qahelp['net12']='<xml><description>Check that File And Print Services has been disabled on all adapters, except for those specified</description><pass>File And Print Services are disabled correctly</pass><fail>File And Print Services are enabled</fail><applies>All</applies></xml>'
...
$script:qahelp['sec08']='<xml><description>Check system is not caching domain credentials</description><pass>Domain credential caching is disabled</pass><fail>Domain credential caching is enabled / Registry setting not found</fail><applies>All</applies></xml>'
...
$script:qahelp['sec16']='<xml><description>Returns a list of ports that are open, excluding anything lower than 1024 and higher than 49152.Will also exclude other well known ports</description><pass>No extra ports are open</pass><fail>One or more extra ports are open</fail><na>This check is for local servers only</na><applies>All</applies></xml>'
...
$script:qahelp['sys13']='<xml><description>Checks that the server is a member of the domain</description><pass>Server is a domain member</pass><warning>This is a workgroup server, is this correct.?</warning><applies>All</applies></xml>'
...
$script:qahelp['sys18']='<xml><description>Check that the current server OU path is not in the default location(s).</description><pass>Server not in default location</pass><fail>Server is in default location</fail><na>Not a domain joined server</na><applies>All</applies></xml>'
