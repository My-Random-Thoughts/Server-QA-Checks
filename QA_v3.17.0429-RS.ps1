...
[array]$script:qaChecks = ('c-acc-01-local-users','c-acc-02-local-account-names','c-acc-03-local-admins','c-acc-04-local-groups','c-acc-05-service-logon-accounts',
                           ...
                           'c-net-01-ipv6-status','c-net-01-no-ipv6','c-net-02-unused-network-interfaces','c-net-03-network-adapter-labels','c-net-04-binding-order',
                           'c-net-05-network-speed-duplex','c-net-06-network-agent','c-net-07-network-teaming','c-net-08-management-adapter','c-net-09-static-routes',
                           'c-net-10-power-management','c-net-11-dns-settings','c-net-12-fileprint-services','c-reg-01-local-time','c-reg-02-timezone','c-reg-03-location',
                           'c-reg-04-language','c-sec-01-schannel-p1-ciphers','c-sec-02-schannel-p2-hashes','c-sec-03-schannel-p3-keyexchangealgorithms','c-sec-04-schannel-p4-protocols',
                           ...
                           'c-sec-14-firewall-rules','c-sec-15-firewall-state','c-sec-16-open-ports','c-sys-01-pending-reboot','c-sys-02-windows-license','c-sys-03-services-not-started',
                           ...
                           'c-sys-10-print-spooler','c-sys-11-autorun-disabled','c-sys-12-snmp-configuration','c-sys-13-domain-member','c-sys-14-power-plan',
                           'c-sys-15-hibernation','c-sys-16-remote-desktop','c-sys-17-terminal-services-licenced','c-sys-18-check-current-ou','c-vhv-01-tools-version',
                           'c-vmw-01-tools-version','c-vmw-02-time-sync','c-vmw-03-nic-type','c-vmw-04-lsi-sas-controller','c-vmw-05-scsi-drive-count','c-vmw-06-total-vm-size',
                           'c-vmw-07-cd-dvd-floppy-mounted')
...
<#
    DESCRIPTION:
        Check IPv6 has been unbound on all active NICs, or disabled globally.
    ...
    RESULTS:
        PASS:
            IPv6 {state} globally
            IPv6 {not-state} globally, but {state} on all NICs
        WARNING:
        FAIL:
            IPv6 {not-state} globally, and NIC(s) found with IPv6 {not-state}
        MANUAL:
        NA:
...
Function c-net-01-ipv6-status
{
    ...
    If ($global:appSettings['IPv6State'] -eq 'Disabled')
    {
        [string]$lookingFor = '-1'
        [string]$stateGood  = 'disabled'
        [string]$stateBad   = 'enabled'
    }
    Else
    {
        [string]$lookingFor = '0'
        [string]$stateGood  = 'enabled'
        [string]$stateBad   = 'disabled'
    }

    If ($keyval -eq $lookingFor)    # All Disabled
    {
        $result.result  = $script:lang['Pass']
        $result.message = "IPv6 $stateGood globally"
    }
    Else
    {
        $result.result  = $script:lang['Fail']
        $result.message = "IPv6 $stateBad globally"
...
$script:qahelp['net01']='<xml><description>Check IPv6 has been unbound on all active NICs, or disabled globally.</description><requiredinputs>IPv6State - "Enabled|Disabled" - State of the IPv6 protocol for each network adapter!n</requiredinputs><pass>IPv6 {state} globally!nIPv6 {not-state} globally, but {state} on all NICs!n</pass><fail>IPv6 {not-state} globally, and NIC(s) found with IPv6 {not-state}!n</fail><applies>All Servers!n</applies></xml>'
$script:qahelp['net01']='<xml><description>Check IPv6 has been unbound on all active NICs, or disabled globally.</description><pass>IPv6 disabled globally!nIPv6 enabled globally, but disabled on all NICs!n</pass><fail>IPv6 enabled globally, and NIC(s) found with IPv6 enabled!n</fail><applies>All Servers!n</applies></xml>'
