...
Function c-sys-14-power-plan
{
    ...
    Try
    {
        If ((Check-NameSpace -ServerName $serverName -NameSpace 'ROOT\Cimv2\Power') -eq $true)
        {
            ...
            [string]$check = Get-WmiObject -ComputerName $serverName -Query $query -Namespace ROOT\Cimv2\Power -ErrorAction SilentlyContinue | Select-Object -ExpandProperty ElementName
        }
