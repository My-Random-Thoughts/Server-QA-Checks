...
Function Start-QAProcess
{
    ...
    [boolean]$noping  = ($PSCmdlet.MyInvocation.BoundParameters.ContainsKey('DoNotPing'))
    ...
    If ($noping -eq $true)           { $NoPingMessage = 'Ignoring Server Connection Checks' }
    ...
    ForEach ($server In $script:servers)
    {
        ...
        If (($noping -eq $true) -or ((Test-Connection -ComputerName $server -Quiet -Count 1) -eq $true))
        {
            ...
            If (($noping -eq $true) -or ((Check-Port -ServerName $server -Port 135) -eq $true))
            {
