...
[array]$script:qaChecks = ('c-acc-01-local-users','c-acc-02-local-account-names','c-acc-03-local-admins','c-acc-04-local-groups','c-acc-05-service-logon-accounts',
                           ...
                           'c-sys-19-hp-smh-version','c-sys-20-dell-oma-version','c-vhv-01-tools-version','c-vmw-01-tools-version','c-vmw-02-time-sync','c-vmw-03-nic-type',
                           'c-vmw-04-lsi-sas-controller','c-vmw-05-scsi-drive-count','c-vmw-06-total-vm-size','c-vmw-07-cd-dvd-floppy-mounted')
...
#            $regKey = $reg.OpenSubKey('Software\Wow6432Node\Network Associates\ePolicy Orchestrator\Agent')
#            If ($regKey) { $slVal = $regKey.GetValue('ePOServerList') }    #    {server}|{ip}|{port};{server}|{ip}|{port};...
...
Function c-sys-19-hp-smh-version
{
    ...
    If (isHPServer -eq $true)
    {
        Try
        {
            [string]$state = (Get-Service -ComputerName $serverName -DisplayName 'HP System Management Homepage' -ErrorAction Stop | Select-Object -ExpandProperty StartType)
        }
        Catch
        {
            $result.result  = $script:lang['Fail']
...
Function c-sys-20-dell-oma-version
{
    ...
    If (isDELLServer -eq $true)
    {
        Try
        {
            [string]$state = (Get-Service -ComputerName $serverName -DisplayName 'DSM SA Shared Services' -ErrorAction Stop | Select-Object -ExpandProperty StartType)
        }
        Catch
        {
            $result.result  = $script:lang['Fail']
