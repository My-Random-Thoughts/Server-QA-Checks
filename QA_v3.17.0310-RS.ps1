...
Function c-vmw-09-check-vosot-services-tasks
{
    ...
    If ((Check-VMware $serverName) -eq $true)
    {
        Try
        {
            ...
            [string[]]$osType  = ('Microsoft', 'Standard', 'Professional', 'Enterprise')
            [string]  $query   = "SELECT Caption, OSArchitecture, Version, ProductType FROM Win32_OperatingSystem"
            [object]  $osCheck = Get-WmiObject -ComputerName $serverName -Query $query -Namespace ROOT\Cimv2 -ErrorAction Stop | Select-Object Caption, OSArchitecture, Version, ProductType
            $osType | ForEach { $osCheck.Caption = (($osCheck.Caption).Replace($_, '')).Trim() }
            ...
            [object[]]$service  = Get-WmiObject -ComputerName $serverName -Query $query -Namespace ROOT\Cimv2 -ErrorAction Stop | Select-Object Name, Caption, StartMode
...
$script:qahelp['sys05']='<xml><description>Check System Event Log and ensure no errors or warnings are present in the last 14 days.  If found, will return the latest y entries</description><requiredinputs>GetLatestEntriesAge   - Return all entries for this number of days|Integer!nGetLatestEntriesCount - Return this number of entries|Integer!n</requiredinputs><pass>No errors found in system event log!n</pass><warning>Errors were found in the system event log!n</warning><applies>All Servers!n</applies></xml>'
$script:qahelp['sys06']='<xml><description>Check Application Event Log and ensure no errors or warnings are present in the last 14 days.  If found, will return the latest y entries</description><requiredinputs>GetLatestEntriesAge   - Return all entries for this number of days|Integer!nGetLatestEntriesCount - Return this number of entries|Integer!n</requiredinputs><pass>No errors found in application event log!n</pass><warning>Errors were found in the application event log!n</warning><applies>All Servers!n</applies></xml>'
